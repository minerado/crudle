...
class CRUDMixin:
    ...
    def update(self, db: Session, on_update_assocs=DEFAULT_ON_UPDATE_ASSOC, **kwargs):
        """Update an instance in the database."""
        return self.__update(db, self, on_update_assocs=on_update_assocs, **kwargs)
                ...
                for item in v:
                    if hasattr(item, "id") and item.id:
                        ...
                        if isinstance(item, dict):
                            ...
                            cls._set_attributes_from_dict(nested_model, item, db)
    ...
    @classmethod
    def _set_attributes_from_dict(cls, model, data_dict, db, on_update="nilify_all"):
        ...
        for k, v in data_dict.items():
            ...
            if k in relationship_map and isinstance(v, dict):
                ...
                if v.get("id"):
                    ...
                    cls._set_attributes_from_dict(association, v, db, on_update)
                ...
                for item in v:
                    if hasattr(item, "id") and item.id:
                        ...
                        if isinstance(item, dict):
                            ...
                            cls._set_attributes_from_dict(
                                nested_model, item, db, on_update
                            )
                            association.append(nested_model)
                        else:
                            association.append(item)
...
class CRUDMixinHelper:
    ...
    def handle_relationship(
        ...
        if isinstance(relationship, list):
            ...
            CRUDMixinHelper.validate_conflicting_operations(
                values, relationship_name, db, model, on_update
            )

            # Validate all association values (including None)
            for item in values:
                CRUDMixinHelper.validate_association_value(item, relationship_name)
                ...
                for item in values:
                    if isinstance(item, dict):
                        ...
                        if "id" in item and item["id"]:
                            ...
                            CRUDMixinHelper.update_association(
                                association, item, foreign_key
                            )
            ...
            for item in values:
                if isinstance(item, dict):
                    # Don't filter None values since we want to handle them in update_association
                    # item = CRUDMixinHelper.filter_none_values(item)
                    ...
                    if "id" in item and item["id"]:
                        ...
                        CRUDMixinHelper.update_association(
                            association, item, foreign_key, on_update
                        )
                        associations.append(association)
                    else:
            ...
            if values is None:
                ...
                CRUDMixinHelper.validate_association_value(values, relationship_name)
    ...
    @staticmethod
    def update_association(association, data, foreign_key, on_update="nilify_all"):
        """Update an association with data, handling nested relationships."""
        relationship_map = {
            k: v for k, v in association.__mapper__.relationships.items()
        }
        ...
        for key, value in data.items():
            ...
            if key in relationship_map and isinstance(value, dict):
                ...
                if value.get("id"):
                    ...
                    related_obj = association._sa_instance_state.session.execute(
                        stmt
                    ).scalar_one()
                    CRUDMixinHelper.update_association(
                        related_obj, value, "id", on_update
                    )
                    setattr(association, key, related_obj)
                else:
                    # Create new relationship
                    related_obj = model_entity()
                    CRUDMixinHelper._set_attributes_from_dict(
                        related_obj,
                        value,
                ...
                if existing_assoc is not None:
                    if on_update == "delete_all":
                        ...
                        association._sa_instance_state.session.delete(existing_assoc)
                        association._sa_instance_state.session.flush()
                    elif on_update == "raise":
                        # Raise error if trying to set to None when on_update=raise
                        raise IntegrityError(
