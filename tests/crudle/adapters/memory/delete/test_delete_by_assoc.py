"""Delete_by association filters — Memory adapter.

Twin of SQLAlchemy ``delete/test_delete_by_assoc.py``.
"""
...
class TestDeleteByAssoc:
    def test_has_many(self, db):
        ...
        assert db.delete_by(ItemList, **{"items.color": "red"}).id == keep.id
    ...
    def test_belongs_to(self, db):
        ...
        assert db.delete_by(Item, **{"item_type.name": "Electronics"}).id == keep.id
    ...
    def test_many_to_many(self, db):
        ...
        assert db.delete_by(Item, **{"tags.name": "sale"}).id == gadget.id
        assert db.count(Item) == 1
    ...
    def test_deep_path(self, db):
        ...
        assert (
            db.delete_by(ItemList, **{"items.tags.name": "expensive"}).id == keep.id
        )
    ...
    def test_multiple_parents_matching_assoc_raises(self, db):
        ...
        with pytest.raises(MultipleResultsFound):
            db.delete_by(ItemList, **{"items.color": "red"})
