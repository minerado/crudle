"""Get_by custom ``Queries`` filters — SQLAlchemy adapter.
...
class TestGetByCustom:
    def test_is_expensive(self, db):
        ...
        assert Item.get_by(db, is_expensive=True).id == keep.id
    ...
    def test_custom_and_filter(self, db):
        ...
        assert Item.get_by(db, is_expensive=True, color="red").id == keep.id
