"""Get_by text-search (`q`) — SQLAlchemy / Postgres FTS.
...
class TestGetByQ:
    ...
    def test_bare_search(self, postgres_db):
        ...
        with search_fields("name"):
            assert Item.get_by(postgres_db, search="Apple").id == keep.id
